...
{
    [Route("open/doc/member/api")]
    [Authorize]
    [ApiController]
    public class ApiMemberController(ApiRepository repository) : JsonController
    {
        [HttpGet]
        [Route("")]
        ...
        {
            var res = repository.EditSelf(id);
            if (res.Succeeded)
            {
        ...
        [ProducesResponseType(typeof(FailureResponse), 404)]
        public IActionResult Save([FromBody] ApiForm form)
        {
            var res = repository.SaveApi(form);
            if (res.Succeeded)
            {
                return Render(res.Result);
            }
